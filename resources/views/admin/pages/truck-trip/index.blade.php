...
<div class="col-12 mt-3 bg-white rounded p-3">
    <div class="table-responsive">
        <table class="table table-striped m-b-0" id="myTable">
            ...
            <tbody>
                @foreach ($truckTripCategories as $index => $truckTripCategory)
                <tr>
                    <td>{{$index+1}}</td>
                    <td>{{$truckTripCategory->name}}</td>
                    <td class="with-btn" nowrap="">
                        <a href="{{route('admin.truck-trip-category.edit', $truckTripCategory->id)}}"
                        ...
                        <form id="language{{ $index }}"
                            action="{{ route('admin.truck-trip-category.destroy', $truckTripCategory->id) }}"
